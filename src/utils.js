...
const formatDate = (date: DateLike): string => format(date, 'yyyy-MM-dd');

const getSubtractionFn = (unit: Unit): ((DateLike, number) => Date) => {
...
const getDiffFn = (unit: Unit): ((DateLike, DateLike) => number) => {
...
const getStartOfFn = (unit: Unit): (DateLike => Date) => {
...
const getEndOfFn = (unit: Unit): (DateLike => Date) => {
...
const parseTimezoneStrToHours = (tz: string): number => {
...
const normalizeOffsetToMs = (offset: number | string): number => {
...
const getMsDiffFromUTC = (offset: number | string): number => (
...
const applyOffset = (offset: number | string, date: DateLike): Date => {

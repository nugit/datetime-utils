...
function migrateLegacyPeriodString(periodString) {
  ...
  var match = periodString.match(LAST_RANGE_REGEX_LEGACY);

  if (match) {
    return "last_".concat(match[1], "_").concat(match[2]);
...
function retrievePeriodParams(periodOrKey) {
  ...
  match = periodOrKey.match(TILL_YESTERDAY_REGEX); // TODO: change till_tomorrow to till_yesterday

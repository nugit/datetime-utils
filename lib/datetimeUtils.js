...
var formatDate = function formatDate(date) {
  return format_1(date, 'YYYY-MM-DD');
};
...
function retrievePredefindedDateRange(key) {
  ...
  if (key === 'yesterday') {
    var yesterday = formatDate(sub_days(base, 1));
    return {
      start: yesterday,
      end: yesterday
    };
  ...
  if (key === 'year_to_date') {
    return {
      start: formatDate(set_day_of_year(base, 1)),
      end: formatDate(base)
    };
  }
  ...
  if (key === 'all_time') {
    return {
      start: formatDate(sub_years(base, 3)),
      end: formatDate(base)
    };
  }
...
function retrieveLastRelativePeriod(num, unit) {
  ...
  if (unit === 'day') {
    return {
      start: formatDate(sub_days(base, num)),
      end: formatDate(sub_days(base, 1))
    };
  }

  if (unit === 'week') {
    return {
      start: formatDate(set_day(sub_weeks(base, num), 1)),
      end: formatDate(set_day(sub_weeks(base, 1), 7))
    };
  }

  if (unit === 'month') {
    return {
      start: formatDate(set_date(sub_months(base, num), 1)),
      end: formatDate(set_date(base, 0))
    };
  }

  if (unit === 'quarter') {
    return {
      start: formatDate(set_date(sub_months(base, num * 3 + get_month(base) % 3), 1)),
      end: formatDate(set_date(add_months(sub_months(base, 3 * num), 3 - get_month(base) % 3), 0))
    };
  }

  if (unit === 'year') {
    return {
      start: formatDate(set_day_of_year(sub_years(base, num), 1)),
      end: formatDate(set_date(set_month(sub_years(base, 1), 11), 31))
    };
  }
}
...
function retrieveThisRelativePeriod(unit) {
  ...
  if (unit === 'week') {
    return {
      start: formatDate(set_day(base, 1)),
      end: formatDate(set_day(base, 7))
    };
  }

  if (unit === 'month') {
    return {
      start: formatDate(set_date(base, 1)),
      end: formatDate(base)
    };
  }
  ...
  if (unit === 'quarter') {
    return {
      start: formatDate(set_date(sub_months(base, get_month(base) % 3), 1)),
      end: formatDate(set_date(add_months(base, 3 - get_month(base) % 3), 0))
    };
  }
  ...
  if (unit === 'year') {
    return {
      start: formatDate(set_day_of_year(base, 1)),
      end: formatDate(last_day_of_year(base))
    };
  }
}
...
function retrievePeriod(periodOrKey) {
  ...
  if (params && params.type === 'till_tomorrow') {
    return {
      start: params.start,
      end: formatDate(sub_days(baseDate, 1))
    };
  }
...
var getSubtractionFn = function getSubtractionFn(unit) {
  if (unit === 'year') return sub_years;
  if (unit === 'month') return sub_months;
  if (unit === 'quarter') return function (date, nb) {
    return sub_months(date, nb * 3);
  };
  if (unit === 'week') return sub_weeks;
  return sub_days;
};
...
function calculateAutoCompare(periodOrKey) {
  ...
  if (params && params.type === 'this') {
    ...
    return {
      ...
      period: {
        ...
        end: formatDate(sub_days(period.start, 1))
      }
    };
  }
  ...
  if (periodOrKey === 'year_to_date') {
    return {
      label: "Previous Year",
      period: {
        start: formatDate(set_day_of_year(sub_years(baseDate, 1), 1)),
        end: formatDate(end_of_year(sub_years(baseDate, 1)))
      }
    };
  }
  ...
  var span = difference_in_days(end, start) + 1;
  var compareStart = sub_days(start, span);
  var compareEnd = sub_days(start, 1);
  var endTomorrow = add_days(end, 1); //handle whole month date range

  if (get_date(start) === 1 && get_date(endTomorrow) === 1) {
    var monthdiff = (get_year(endTomorrow) - get_year(start)) * 12 + get_month(endTomorrow) - get_month(start);
    compareStart = sub_months(period.start, monthdiff);
  } //handle whole year date range


  if (get_month(start) === 1 && get_date(start) === 1 && get_month(endTomorrow) === 1 && get_date(endTomorrow) === 1) {
    compareStart = sub_years(start, get_year(endTomorrow) - get_year(start));
  }
...
function retrieveComparePeriod(period) {
  ...
  if (comparison === '12_months_ago') {
    ...
    return {
      start: formatDate(sub_years(yearPeriod.start, 1)),
      end: formatDate(sub_years(yearPeriod.end, 1))
    };
  }

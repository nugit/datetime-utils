...
var parseTimezone = function parseTimezone(tz) {
...
var normalizeOffset = function normalizeOffset(offset) {
  // It's not a number: we assume it's in the shape of `+08:00` and parse it
  if (typeof offset !== 'number') return parseTimezone(offset) * 3600 * 1000; // Offset in seconds: we convert to miliseconds

  if (Math.abs(offset) > 15) return offset * 1000; // Othewise we assume it's hours: we convert to seconds then miliseconds

  return offset * 3600 * 1000;
}; // Gets the number of miliseconds separating a date plus an offset from UTC
// :: (Int | String) -> Object -> Int

var getTzDiff = function getTzDiff(offset, date) {
...
var applyOffset = function applyOffset(offset, dateStr) {
  ...
  var tzDiff = getTzDiff(offset, dateObj);
